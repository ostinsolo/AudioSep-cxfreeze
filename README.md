# AudioSep cx_Freeze — Separate Anything You Describe

**Standalone frozen runtime** for [AudioSep](https://github.com/Audio-AGI/AudioSep) (language-queried audio separation).  
Repo: [github.com/ostinsolo/AudioSep-cxfreeze](https://github.com/ostinsolo/AudioSep-cxfreeze)

- **Releases**: Built executables for **Windows (CUDA/CPU)** and **macOS ARM (MPS)**. Tag `v*` or run *Release AudioSep cx_Freeze* workflow to create a release.
- **Requirements**: `requirements-inference-mps.txt` (Mac ARM), `requirements-inference-intel.txt` (Mac Intel), `requirements-inference-windows-cuda.txt`, `requirements-inference-windows-cpu.txt`.
- **Build**: `python build_dsu_audiosep.py` → `dist/dsu-audiosep/`. Runtime scripts: `build_runtime_mac_mps.sh`, `build_runtime_mac_intel.sh`.
- **Models**: See [MODEL_DOWNLOADS.md](MODEL_DOWNLOADS.md) for checkpoint URLs.
