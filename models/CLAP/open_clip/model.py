...
class CLAP(nn.Module):
    def __init__(
        ...
        if text_cfg.model_type == "transformer":
            ...
            bert_dir = os.environ.get("AUDIOSEP_BERT_DIR", "bert-base-uncased")
            self.text_branch = BertModel.from_pretrained(bert_dir, local_files_only=True)
            self.text_transform = MLPLayers(
                units=[
                    self.joint_embed_shape,
                    self.joint_embed_shape,
            ...
            roberta_dir = os.environ.get("AUDIOSEP_ROBERTA_DIR", "roberta-base")
            self.text_branch = RobertaModel.from_pretrained(roberta_dir, local_files_only=True)
            ...
            bart_dir = os.environ.get("AUDIOSEP_BART_DIR", "facebook/bart-base")
            self.text_branch = BartModel.from_pretrained(bart_dir, local_files_only=True)
            self.text_transform = MLPLayers(
                units=[
                    self.joint_embed_shape,
                    self.joint_embed_shape,
