...
class CLAP_Encoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        env_ckpt = os.environ.get("AUDIOSEP_CLAP_CKPT")
        if env_ckpt:
            pretrained_path = env_ckpt
        elif not os.path.isabs(pretrained_path):
            base_dir = os.environ.get("AUDIOSEP_BASE_DIR")
            if base_dir:
                cand = os.path.join(base_dir, pretrained_path)
                if os.path.exists(cand):
                    pretrained_path = cand
        self.device = "cpu"
        self.precision = "fp32"
        ...
        self.tokenize = RobertaTokenizer.from_pretrained(roberta_dir, local_files_only=True)
